...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 3, "cvodeieq");
 	...
 	ivoc_help("help ?1 NaF D:/NEURON models/DCN LuthmanEtAl2011 Stimulation Ef/mod_files/NaF.mod\n");
...
static int _reset;
static char *modelname = "Fast sodium current (NaF) of deep cerebellar nucleus (DCN) neuron";
...
static int  _f_rate ( _threadargsprotocomma_ double _lv ) {
   minf = 1.0 / ( 1.0 + exp ( ( _lv + 45.0 ) / - 7.3 ) ) ;
   taum = 5.83 / ( exp ( ( _lv - ( 6.4 ) ) / - 9.0 ) + exp ( ( _lv + 97.0 ) / 17.0 ) ) + 0.025 ;
   taum = taum / qdeltat ;
   hinf = 1.0 / ( 1.0 + exp ( ( _lv + 42.0 ) / 5.9 ) ) ;
   tauh = 16.67 / ( exp ( ( _lv - 8.3 ) / - 29.0 ) + exp ( ( _lv + 66.0 ) / 9.0 ) ) + 0.2 ;
   tauh = tauh / qdeltat ;
    return 0; }
