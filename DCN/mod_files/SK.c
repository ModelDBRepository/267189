...
#define qdeltat qdeltat_SK
 double qdeltat = 1;
#define usetable usetable_SK
 double usetable = 1;
 ...
 "qdeltat_SK", &qdeltat_SK,
 "usetable_SK", &usetable_SK,
 0,0
};
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 4, "cvodeieq");
 	...
 	ivoc_help("help ?1 SK D:/NEURON models/DCN LuthmanEtAl2011 Stimulation Ef/mod_files/SK.mod\n");
...
static int _reset;
static char *modelname = "Small conductance calcium dependent potassium current (SK) of deep cerebellar nucleus (DCN) neuron";
...
/*CVODE*/
 ...
 static int rate(double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt, double _lcai) { 
#if 0
_check_rate(_p, _ppvar, _thread, _nt);
#endif
 _n_rate(_p, _ppvar, _thread, _nt, _lcai);
 return 0;
 }

 static void _n_rate(double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt, double _lcai){ int _i, _j;
 double _xi, _theta;
 if (!usetable) {
 _f_rate(_p, _ppvar, _thread, _nt, _lcai); return; 
}
 _xi = _mfac_rate * (_lcai - _tmin_rate);
 if (isnan(_xi)) {
  zinf = _xi;
  tauz = _xi;
  return;
...
static int  _f_rate ( _threadargsprotocomma_ double _lcai ) {
   zinf = _lcai * _lcai * _lcai * _lcai / ( _lcai * _lcai * _lcai * _lcai + 8.1e-15 ) ;
   if ( _lcai < 0.005 ) {
     tauz = 1.0 - ( 186.67 * _lcai ) ;
     }
   else {
     tauz = 0.0667 ;
     }
...
static void _initlists(){
 ...
 _slist1[0] = &(z) - _p;  _dlist1[0] = &(Dz) - _p;
   ...
   _t_tauz = makevector(301*sizeof(double));
_first = 0;
}
